...
def load_all():
    ...
    for _, r in movies.iterrows():
        db.add(Movie(movieId=int(r.movieId), title=str(r.title), genres=str(r.genres)))

    links = pd.read_csv("database_files/links.csv")
    for _, r in links.iterrows():
        db.add(Link(movieId=int(r.movieId),
